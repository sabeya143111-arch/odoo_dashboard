...
def fetch_dead_stock(threshold_days=60, system_key="SWAG",
    ...
    try:
        ...
        _prog(0.15, t(f"Step 1/4 — Found {len(all_pids):,} in-stock products.", f"الخطوة 1/4 — تم العثور على {len(all_pids):,} منتج في المخزون."))
        _prog(0.20, t(f"Step 2/4 — Checking recent sales (last {threshold_days} days)...", f"الخطوة 2/4 — فحص المبيعات الأخيرة (آخر {threshold_days} يوم)..."))
        ...
        _prog(0.35, t(f"Step 2/4 — {len(dead_pids):,} items have no recent sale (dead candidates).", f"الخطوة 2/4 — {len(dead_pids):,} صنف بلا مبيعات حديثة (مرشحون للركود)."))
        if not dead_pids:
            _prog(1.0, t("All products sold recently — no dead stock!","جميع المنتجات بيعت مؤخراً — لا مخزون راكد!"))
            return empty, False
        CHUNK      = 200
        n_chunks   = max(1, (len(dead_pids) + CHUNK - 1) // CHUNK)
        prog_start = 0.35
        prog_end   = 0.85
        _prog(prog_start, t(f"Step 3/4 — Fetching sale history for {len(dead_pids):,} items ({n_chunks} batches)...", f"الخطوة 3/4 — جلب تاريخ المبيعات لـ {len(dead_pids):,} صنف ({n_chunks} دفعة)..."))
        for batch_idx, i in enumerate(range(0, len(dead_pids), CHUNK)):
            ...
            _prog(pct, t(f"Step 3/4 — Batch {batch_num}/{n_chunks} ({min(i+CHUNK, len(dead_pids)):,}/{len(dead_pids):,} items)...", f"الخطوة 3/4 — الدفعة {batch_num}/{n_chunks} ({min(i+CHUNK, len(dead_pids)):,}/{len(dead_pids):,} صنف)..."))
            try:
                ...
                is_partial = True
                _prog(pct, t(f"⚠️ Batch {batch_num} failed ({chunk_err}) — continuing with partial results.", f"⚠️ فشلت الدفعة {batch_num} — متابعة بنتائج جزئية."))
                continue
            if not sol_chunk: continue
            oids = list({ln["order_id"][0] for ln in sol_chunk if isinstance(ln.get("order_id"),list)})
        ...
        _prog(1.0, t(f"Done — {len(df):,} dead stock items found.", f"اكتمل — تم العثور على {len(df):,} صنف راكد."))
...
def build_size_pivot(df, mc_col, qc_col, sc_col, pc_col, thr=0):
    ...
    work[["_base","_size"]] = work[mc_col].apply(lambda c: pd.Series(_extract_size(str(c))))
    sized = work[work["_size"] != ""].copy()
    if sized.empty: return None, []
    idx_cols = [c for c in [sc_col, "_base"] if c in sized.columns]
    pivot = (sized.pivot_table(index=idx_cols, columns="_size",
                               values="_qty_num", aggfunc="sum", fill_value=0).reset_index())
    pivot.columns.name = None
    if pc_col in sized.columns:
        price_map = (sized.groupby("_base")[pc_col].first().reset_index()
    ...
    extra_sizes = [c for c in pivot.columns if c not in idx_cols + ["_price","_base"] and c not in _SIZE_ORDER]
    ...
    pivot    = pivot[ordered].sort_values([sys_col_label, base_col_name] if sys_col_label else [base_col_name]).reset_index(drop=True)
        ...
        if sub:
            ...
            with st.spinner(t("Authenticating...","جارٍ التحقق...")):
                try:
                    ...
                    proxy = xmlrpc.client.ServerProxy(f"{_login_url}/xmlrpc/2/common", allow_none=True)
...
def show_dashboard():
    ...
    with st.sidebar:
        ...
        if st.button(t("Logout →","خروج →"), use_container_width=True, type="secondary"):
            do_logout()
        st.divider()
        st.markdown(f"<div class='section-tag'>{t('Search Mode','وضع البحث')}</div>", unsafe_allow_html=True)
        ...
        if et!=st.session_state.search_exact:
            ...
            st.rerun()
        st.divider()
        st.markdown(f"<div class='section-tag'>{t('Low Stock Alert','تنبيه المخزون')}</div>", unsafe_allow_html=True)
        thr = st.number_input(t("Threshold (qty ≤)","الحد (كمية ≤)"), min_value=0, max_value=1000,
    ...
    for _k in SYSTEM_KEYS:
        ...
        _sys_pills += f"<div class='sp sp-{_cls}'><div class='sd sd-{_cls}'></div><span class='sn sn-{_cls}'>{_dname}</span><span class='sb sb-{_cls}'>{_lbl}</span></div>"
    ...
    if _snap:
        ...
        _ago_str = (f"{int(_ago_s//3600)}h ago" if _ago_s>=3600 else f"{int(_ago_s//60)}m ago" if _ago_s>=60 else "just now")
        _last_html = ("<div class='snap-last'><div class='sl-label'>"+t("Last Run","آخر تشغيل")+"</div>"+
    ...
    with p2:
        emode = None
        if updf:
            emode = st.radio(t("Extract mode","وضع الاستخراج"), [t("Main models","موديلات رئيسية"), t("With sizes","مع المقاسات")], horizontal=True)
    if updf:
        fbytes = updf.read()
        ...
        if raw:
            ...
            unique  = get_unique_base_models(raw) if is_main else list(dict.fromkeys([item["code"] for item in raw]))
            ...
            ca,cb = st.columns(2)
            with ca:
                if st.button(t("Total Stock","مخزون إجمالي"), type="primary", use_container_width=True, key="pt"):
                    st.session_state.pdf_codes = unique_codes
                    st.session_state.pdf_mode  = "total"; st.rerun()
            with cb:
                if st.button(t("Branch-wise","حسب الفرع"), type="secondary", use_container_width=True, key="pb"):
                    st.session_state.pdf_codes = unique_codes
    ...
    with L:
        if not st.session_state.search_exact:
            st.markdown(f"<div class='info-banner'>{t('Variant mode — XP6013 finds all sizes','وضع المتغيرات — XP6013 يجد جميع المقاسات')}</div>", unsafe_allow_html=True)
        else:
            st.markdown(f"<div class='warn-banner'>{t('Exact match — identical codes only','تطابق تام — رموز مطابقة فقط')}</div>", unsafe_allow_html=True)
        ...
        if sr:
            with st.expander(t("Reorder Settings","إعدادات إعادة الطلب"), expanded=True):
                rx,ry = st.columns(2)
                with rx:
                    st.session_state.reorder_target_days = st.slider(t("Target days cover","أيام التغطية"), 7, 180, st.session_state.reorder_target_days)
                with ry:
    ...
    with R:
        ...
        if not snap:
            st.markdown(f"<div class='info-banner'>{t('Run a comparison first.','قم بتشغيل مقارنة أولاً.')}</div>", unsafe_allow_html=True)
        else:
            on = sum(1 for v in stats.values() if v=="OK")
            st.markdown(f"<div class='snap-card'><b>{t('Time','الوقت')}</b> &nbsp;{snap.get('time','—')}<br><b>{t('Models','الموديلات')}</b> &nbsp;{snap.get('models','—')}<br><b>{t('Online','متصل')}</b> &nbsp;{on}/{len(SYSTEM_KEYS)}<br><b>{t('Rows','الصفوف')}</b> &nbsp;{snap.get('rows','—')}</div>", unsafe_allow_html=True)
    ...
    if run_codes is not None:
        ...
        for _pur_key in _PUR_SYSTEMS:
            ...
            if not _pur_models:
                continue
            with st.spinner(t(f"Fetching purchase totals ({_pur_name})...", f"جلب إجمالي المشتريات ({_pur_name})...")):
                _pur_df = get_purchase_summary_by_model(tuple(_pur_models), sd.strftime("%Y-%m-%d"), ed.strftime("%Y-%m-%d"), system_key=_pur_key)
            if not _pur_df.empty:
                _pur_df2 = _pur_df.rename(columns={"Model Code": mc_loc})
                _tmp = tdf.merge(_pur_df2[[mc_loc,"Purchase Qty"]].rename(columns={"Purchase Qty":"_pur_tmp"}), on=mc_loc, how="left")
    ...
    if thr>0 and qc2 in ok.columns:
        ...
        if not low.empty:
            ...
            det = " · ".join(f"{r.get(mc2,'?')} @ {r.get(sc2,'?')} ({r.get(qc2,0)})" for _,r in low.head(6).iterrows())
            if len(low)>6: det += f" +{len(low)-6}"
            st.markdown(f"<div class='alert-banner'><div class='alert-dot'></div><div class='alert-txt'>{t('Low Stock','مخزون منخفض')} — {len(low)} items ≤ {thr}</div><div class='mono'>{det}</div></div>", unsafe_allow_html=True)
    ...
    if pc2 in ok.columns:
        ...
        m4.metric(t("Avg Price (SAR)","متوسط السعر ر.س"), f"{vp.mean():,.0f}" if not vp.empty else "—")
        ...
        if not _size_view:
            ...
            if _pivot_df is None or _pivot_df.empty:
                ...
                if _sz_sc in _pivot_df.columns:
                    ...
                    if len(_sys_opts) > 1:
                        _sel_sys_sz = st.multiselect(t("Filter by System","فلتر حسب النظام"), options=_sys_opts, default=_sys_opts, key="sz_sys_filter")
                        if _sel_sys_sz:
                            _pivot_df = _pivot_df[_pivot_df[_sz_sc].isin(_sel_sys_sz)]
                _sz_search = st.text_input(t("Search base model","بحث موديل أساسي"), placeholder="e.g. XP6013", key="sz_search").strip().upper()
                if _sz_search:
                    _bc = t("Base Model","الموديل الأساسي")
                    if _bc in _pivot_df.columns:
                        _pivot_df = _pivot_df[_pivot_df[_bc].str.upper().str.contains(_sz_search, regex=False, na=False)]
                st.caption(f"{len(_pivot_df)} {t('models','موديل')} · {len(_size_cols)} {t('sizes','حجم')} · {t('Red=0 · Amber=Low · Teal=OK','أحمر=صفر · عنبر=منخفض · تيل=كافٍ')}")
...
.swag-tbl tbody td.cf{font-family:'Outfit',monospace;font-size:11px;letter-spacing:0.5px;color:#111;font-weight:500;border-right:1px solid rgba(0,0,0,0.08);}
        ...
        d3.download_button(t("All Systems ↓","كل الأنظمة ↓"), to_excel_bulk(tdf), dl_name("bulk","xlsx"), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
        if _ft is not None and not _ft.empty:
            d4.download_button(t("Filtered ↓","مفلتر ↓"), to_excel(_ft), dl_name("filtered","xlsx"), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
        st.divider()
    ...
    if hb:
        with tabs[ti]:
            ti += 1
            st.markdown(f"<div class='section-tag' style='margin-top:20px;'>{t('Branch-wise Stock','مخزون حسب الفرع')}</div>", unsafe_allow_html=True)
            ...
            if not okb.empty and bc2 in okb.columns and qc2 in okb.columns:
                ...
                if not chart.empty:
                    st.markdown(f"<div class='section-tag'>{t('Qty by Branch','الكميات حسب الفرع')}</div>", unsafe_allow_html=True)
            ...
            b2.download_button("Excel ↓", to_excel(bdf), dl_name("branch","xlsx"), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
    ...
    if ht:
        with tabs[ti]:
            ti += 1
            st.markdown(f"<div class='section-tag' style='margin-top:20px;'>{t('Pending Transfers','النقليات المعلقة')}</div>", unsafe_allow_html=True)
    ...
    if hr:
        with tabs[ti]:
            ...
            st.markdown(f"<div class='section-tag' style='margin-top:20px;'>{t('Reorder Suggestions','اقتراحات إعادة الطلب')}</div>", unsafe_allow_html=True)
            ...
            if not okr.empty:
                ...
                if crit+lo>0:
                    st.markdown(f"<div class='warn-banner'>{crit+lo} {t('products need reordering','منتجات تحتاج إعادة طلب')}</div>", unsafe_allow_html=True)
                sa = st.toggle(t("Show all","عرض الكل"), value=False)
                dr = (okr if sa else okr[okr[CPRI].str.contains("Critical|Low",na=False)] if CPRI in okr.columns else okr)
