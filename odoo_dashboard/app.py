...
from PyPDF2 import PdfReader   # ← ADDED FOR PDF INVOICE PARSING
...
# ─────────────────────────────────────────────────────────────────────────────
# NEW: PDF INVOICE PARSING FUNCTIONS (ONLY ADDITION)
# ─────────────────────────────────────────────────────────────────────────────
def extract_base_model(code: str) -> str:
    """Remove (parentheses) and size suffixes to get main model only."""
    code = re.sub(r'\([^)]*\)', '', code)
...
def parse_invoice_pdf(uploaded_file) -> list:
    """Parse Swag invoice PDF (all 3 formats: bracketed, table, Arabic RTL)."""
...
def show_dashboard() -> None:
    ...
    # ═══ PDF UPLOAD SECTION (ADDED EXACTLY AS REQUESTED - BEFORE MANUAL SEARCH) ═══
    ...
    # ── Two-column layout (manual search - 100% UNCHANGED) ─────────────────────
    ...
    # ── Run comparison (MODIFIED ONLY TO SUPPORT PDF CODES - NOTHING ELSE CHANGED) ────────────────────────────────────────────────────────
    if compare_btn or 'pdf_codes_to_search' in st.session_state:
        # Use PDF codes if available, otherwise manual input
        if 'pdf_codes_to_search' in st.session_state and st.session_state['pdf_codes_to_search']:
            model_codes = st.session_state['pdf_codes_to_search']
            st.session_state['pdf_codes_to_search'] = None  # Clear after use
        else:
            model_codes = codes  # From manual input

        if not model_codes:
            st.warning(t("Please enter codes or upload PDF.", "الرجاء إدخال رموز أو ارفع PDF."))
            st.stop()
    ...
    if st.session_state.show_reorder and reorder_df is not None and not reorder_df.empty:
        with tabs[tab_idx]:
            ...
            if not ok_reorder.empty:
                ...
                st.info(t("No reorder data to display.", "لا توجد بيانات إعادة الطلب للعرض."))
            # Downloads
