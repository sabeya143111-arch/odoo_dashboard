...
def fetch_all_data(
    ...
    def _one(key):
        ...
        try:
            ...
            for p in prods:
                R["total"].append({CS:sn,CM:p.get("default_code") or "—",
            ...
            if need_branch:
                ...
                for q in qs:
                    ...
                    pm=pmap.get(pid,{})
                    R["branch"].append({CS:sn,CB:ln.split("/")[0].strip(),
            ...
            if need_reorder:
                ...
                for l in sl:
                    ...
                    if pid: sm[pid]=sm.get(pid,0)+float(l.get("product_uom_qty") or 0)
                for p in prods:
                    pid=p["id"]; cq=int(p.get("qty_available") or 0)
                    sold=sm.get(pid,0); vel=round(sold/DAYS,2)
                    ...
                    R["reorder"].append({CS:sn,CM:p.get("default_code") or "—",
                                         CPR:p.get("display_name") or "",
