...
.sz-tbl th{
  background:#1A7A82;color:#fff;
  font-size:11px;font-weight:800;
  letter-spacing:2px;text-transform:uppercase;
  padding:12px 14px;text-align:center;
}
.sz-tbl td{
  padding:11px 14px;color:#111827;font-weight:700;
...
.sp-online{background:#D1FAE5;border:2px solid #6EE7B7;}
.sp-offline{background:#F3F4F6;border:2px solid #E5E7EB;}
.sp-error{background:#FEE2E2;border:2px solid #FCA5A5;}
.sp-nodata{background:#FFFBEB;border:2px solid #FDE68A;}
...
.sn{font-size:13px;font-weight:700;letter-spacing:0.5px;}
.sn-online{color:#065F46;}
.sn-offline{color:#6B7280;}
.sn-error{color:#991B1B;}
.sn-nodata{color:#92400E;}
.sb{
...
.snap-last{
  ...
  background:#EEF9FA;border:2px solid #1A7A82;
...
.sl-ago{font-size:13px;font-weight:600;color:#9CA3AF;}
.sl-rows{font-size:14px;font-weight:800;color:#1A7A82;}
...
.sc-pill-ok{
  background:#FFFFFF;border:2px solid #1A7A82;
...
@media (max-width:768px){
  ...
  .sc-hdr{font-size:28px !important;}
  .snap-cards{grid-template-columns:repeat(2,1fr) !important;}
  .sc-val{font-size:26px !important;}
  .stButton button{font-size:10px !important;min-height:44px !important;}
  .section-tag{font-size:9px !important;letter-spacing:2px !important;}
}
...
@media (hover:none){
  ...
  [data-testid="stExpander"] summary{min-height:52px !important;}
...
.swag-wrap,[data-testid="stDataFrame"]{
...
def show_dashboard():
    ...
    if _on_purchase:
        ...
        if _po_df.empty:
            ...
            # ── VENDOR REPORT CARD ────────────────────────────────
            st.markdown(f"<div class='ph-section'>{t('Vendor Report Card','بطاقة تقرير الموردين')}</div>",
                        unsafe_allow_html=True)

            if all(c in _po_df.columns for c in ["Vendor","Qty Ordered","Qty Received","Subtotal SAR","PO Number"]):
                _vrc = (_po_df.groupby("Vendor", as_index=False)
                        .agg({
                            "Subtotal SAR":  "sum",
                            "PO Number":     "nunique",
                            "Qty Ordered":   "sum",
                        ...
                        .rename(columns={"PO Number": "POs"}))
                _vrc["Delivery %"] = (_vrc["Qty Received"] / _vrc["Qty Ordered"] * 100).round(1)
                _vrc["Delivery %"] = _vrc["Delivery %"].clip(0, 100)
                ...
                *{box-sizing:border-box;margin:0;padding:0;}
                ...
                _worst_dp = float(_vrc.iloc[-1]["Delivery %"]) if len(_vrc)>1 else 0
                _worst_v  = _vrc.iloc[-1]["Vendor"] if len(_vrc)>1 else ""
                st.markdown(f"""
                <div style='display:flex;gap:12px;margin-top:12px;flex-wrap:wrap;'>
                  <div style='background:#D1FAE5;border:1.5px solid #6EE7B7;border-radius:10px;
                    padding:10px 16px;flex:1;min-width:180px;'>
                    <div style='font-size:9px;font-weight:800;letter-spacing:2px;
                      text-transform:uppercase;color:#065F46;margin-bottom:3px;'>
                      ⭐ {t("Best Performer","أفضل مورد")}</div>
                    <div style='font-size:16px;font-weight:700;color:#065F46;'>{_best_vendor}</div>
                    <div style='font-size:12px;color:#059669;font-weight:600;'>{_best_dp:.1f}% {t("delivery","تسليم")}</div>
                  </div>
                  {'<div style="background:#FEE2E2;border:1.5px solid #FCA5A5;border-radius:10px;padding:10px 16px;flex:1;min-width:180px;"><div style="font-size:9px;font-weight:800;letter-spacing:2px;text-transform:uppercase;color:#991B1B;margin-bottom:3px;">⚠️ ' + t("Needs Followup","يحتاج متابعة") + '</div><div style="font-size:16px;font-weight:700;color:#991B1B;">' + _worst_v + '</div><div style="font-size:12px;color:#DC2626;font-weight:600;">' + f"{_worst_dp:.1f}%" + " " + t("delivery","تسليم") + '</div></div>' if len(_vrc)>1 and _worst_dp < 95 else ''}
                </div>
                """, unsafe_allow_html=True)
        ...
        def _fetch_inventory(sys_key):
            ...
            try:
                ...
                def _mn(v):
                    ...
                    if isinstance(v,str) and v.strip() and v!="False": return v.strip()
                    return ""

                # Step 1: Get product.product fields
                fmeta=x(db,uid,ak,"product.product","fields_get",[],
                        {"attributes":["string","type"]}) or {}

                # Step 2: Find brand fields — check product.product AND product.template
                tmpl_fmeta=x(db,uid,ak,"product.template","fields_get",[],
                             {"attributes":["string","type"]}) or {}
                ...
                for _bf in ["x_brand_category_id","x_studio_brand_category",
                            ...
                            "x_studio_char_field","x_brand_id"]:
                    if _bf in fmeta: _pp_brand_fields.append(_bf)
                    if _bf in tmpl_fmeta: _pt_brand_fields.append(_bf)

                rf=["id","default_code","display_name","list_price",
                    "categ_id","product_tmpl_id"]+_pp_brand_fields
                ...
                # Step 3: Fetch brands from product.template if needed
                tmpl_brands={}
                if _pt_brand_fields and not _pp_brand_fields:
                    tmpl_ids=list({p["product_tmpl_id"][0]
                                   for p in prods if p.get("product_tmpl_id")})
                    if tmpl_ids:
                        trf=["id"]+_pt_brand_fields
                        for tchunk in [tmpl_ids[i:i+200] for i in range(0,len(tmpl_ids),200)]:
                            tr=x(db,uid,ak,"product.template","read",
                                 [tchunk],{"fields":trf}) or []
                            for t_ in tr:
                                brand=""
                                for _bf in _pt_brand_fields:
                                    brand=_mn(t_.get(_bf,""))
                                    if brand: break
                                if brand: tmpl_brands[t_["id"]]=brand

                # Step 4: Also try x_categ_brand from categ_id name parsing
                pmap={}
                for p in prods:
                    nm=str(p.get("display_name") or "").strip()
                    ...
                    # Try product.product brand fields
                    brand=""
                    for _bf in _pp_brand_fields:
                        brand=_mn(p.get(_bf,""))
                        if brand: break

                    # Try product.template brands
                    if not brand:
                        tmpl_id=(p["product_tmpl_id"][0]
                                 if isinstance(p.get("product_tmpl_id"),list)
                                 else p.get("product_tmpl_id"))
                        brand=tmpl_brands.get(tmpl_id,"")

                    # Fallback: extract from category name
                    categ_nm=_mn(p.get("categ_id","")) or "Other"
                    if not brand:
                        # Try splitting category like "Men / Nike" → "Nike"
                        _cat_parts=[c.strip() for c in categ_nm.split("/")]
                        if len(_cat_parts)>=2:
                            brand=_cat_parts[-1]  # last part often is brand
...
<div class='card'>
  ...
  <table>
    <thead><tr>
      ...
      <th>{t('Brand','الماركة')}</th>
      <th class='r'>{t('Qty','الكمية')}</th>
      <th class='r'>{t('Price','السعر')}</th>
      <th class='r'>{t('Value SAR','القيمة')}</th>
    </tr></thead>
    <tbody>"""
