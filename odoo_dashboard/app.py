...
Version 6.0 — Auth Cache + Exact Invoice + No Limit + Refresh Fix
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# XML-RPC — AUTH CACHED PER SESSION (speed fix)
# ─────────────────────────────────────────────────────────────────────────────
@st.cache_data(ttl=3600, show_spinner=False)
def _auth_cached(url: str, db: str, user: str, api_key: str):
    """Cache auth UID for 1 hour so repeated calls don't re-authenticate."""
    try:
...
# ─────────────────────────────────────────────────────────────────────────────
# PDF PARSING — EXACT CODES, NO LIMIT
# ─────────────────────────────────────────────────────────────────────────────
...
def parse_invoice_pdf(uploaded_file) -> list:
    ...
    found = []
    # Strategy 1: bracketed codes [XP6013-S]
    found.extend(re.findall(r'\[([A-Za-z0-9\-_()]{3,30})\]', full_text))
    # Strategy 2: table row before SR price
    ...
    EXCLUDE = {'SR','VAT','TAX','PCS','QTY','NO','REF','INV','PO','SO','DO','ID','EN','AR','PDF',
               'AED','SAR','USD','KWD','OMR','BHD','JOD','EGP','TRY'}
    seen, unique = set(), []
    for code in found:
        ...
        if code not in seen:
            seen.add(code)
            unique.append(code)
    return unique  # NO LIMIT — return all codes

# ─────────────────────────────────────────────────────────────────────────────
# FETCH: TOTAL STOCK — BATCH ALL CODES IN ONE CALL PER SYSTEM
# ─────────────────────────────────────────────────────────────────────────────
@st.cache_data(ttl=120, show_spinner=False)
def fetch_total_stock_batch(codes_tuple: tuple, exact: bool = False) -> pd.DataFrame:
    """
    Fetch all codes in ONE search_read call per system instead of per-code calls.
    codes_tuple: tuple of model codes (tuple so it's hashable for cache)
    """
    CS  = t("System",     "النظام")
    ...
    if exact:
        ...
        # OR domain for all codes
        domain = ["|"] * (len(codes) - 1) + [["default_code", "=like", f"{c}%"] for c in codes]
        if len(codes) == 1:
            domain = [["default_code", "=like", f"{codes[0]}%"]]
    ...
    for key in SYSTEM_KEYS:
        ...
        if not uid:
            rows.append({CS: sn, CM: "—", CPR: t("⚠️ Auth failed","⚠️ فشل التحقق"),
        ...
        try:
            prods = _exec(cfg["url"], cfg["db"], uid, cfg["api_key"],
                          ...
                          {"fields": ["id","display_name","default_code","qty_available","list_price"],
                           "limit": 2000})
            if not prods:
                rows.append({CS: sn, CM: "—", CPR: t("Not found","غير موجود"),
                             CP: 0.0, CQ: 0, "_status": "NOT_FOUND"})
            else:
                for p in prods:
                    rows.append({
                        CS:  sn,
                        CM:  p.get("default_code") or "—",
...
def fetch_branch_stock_batch(codes_tuple: tuple, exact: bool = False) -> pd.DataFrame:
    ...
    for key in SYSTEM_KEYS:
        ...
        try:
            ...
            # ONE quant call for all products
...
def fetch_transfers_batch(codes_tuple: tuple, exact: bool = False) -> pd.DataFrame:
    ...
    for key in SYSTEM_KEYS:
        ...
        try:
            prods = _exec(cfg["url"], cfg["db"], uid, cfg["api_key"],
                          "product.product", "search_read",
                          [domain], {"fields": ["id","default_code"], "limit": 2000})
            if not prods:
                continue
            prod_ids   = [p["id"] for p in prods]
            ...
            for move in moves:
                ...
                rows.append({CS: sn, CR: pick.get("name") or "—",
...
def fetch_reorder_batch(codes_tuple: tuple, exact: bool = False,
    ...
    CPR   = t("Product",   "المنتج");   CQ    = t("On Hand",   "متوفر")
    ...
    for key in SYSTEM_KEYS:
        ...
        try:
            ...
            prod_ids = [p["id"] for p in prods]
            # ONE sale.order.line call for all products
...
def show_login():
    ...
    with col:
        ...
        st.markdown("<p style='color:#6c757d;margin-top:-10px;'>Real-time Stock & Price across 4 Odoo Systems</p>",
                    unsafe_allow_html=True)
...
def show_dashboard():
    ...
    st.markdown(f"<p style='color:#6c757d;margin-top:-12px;'>{t('Real-time stock & price across 4 Odoo systems','المخزون والسعر عبر 4 أنظمة أودو')}</p>",
                unsafe_allow_html=True)
    st.divider()
    ...
    if uploaded_pdf:
        ...
        if raw:
            ...
            # NO limit — search all codes from invoice

            c1, c2, c3 = st.columns(3)
            c1.metric(t("Raw codes found","رموز مستخرجة"), len(raw))
    ...
    if run_codes is not None:
        ...
        # BATCH CALLS — all codes in one shot per system (very fast)
        ...
        bar.progress(0.75)
        reorder_df  = fetch_reorder_batch(codes_tuple, exact=exact,
                                          ...
                                          reorder_point=st.session_state.reorder_point) if show_reorder else pd.DataFrame()
        bar.progress(1.0)
        bar.empty()
    ...
    if thresh > 0 and qty_col in ok_rows.columns:
        ...
        if not low.empty:
            ...
            st.markdown(
                f"<div class='alert-banner'>🔴 <b>{t('Low Stock Alert','تنبيه مخزون')}:</b> "
    ...
    with tabs[ti]:
        ...
        d3.download_button("📥 Bulk", to_excel_bulk(total_df), dl_name("bulk","xlsx"),  "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
    ...
    with tabs[ti]:
        ...
        if hist_df.empty:
            st.info(t("Run multiple comparisons to track price changes.","قم بتشغيل مقارنات متعددة."))
        else:
    ...
    if has_transfers:
        with tabs[ti]:
            ...
            st.markdown("<div class='info-banner'>Shows draft, waiting, confirmed, ready transfers.</div>",
                        unsafe_allow_html=True)
    ...
    if has_reorder:
        with tabs[ti]:
            ...
            if not ok_r.empty:
                ...
                if crit+low > 0:
                    st.markdown(
                        f"<div class='alert-banner'>🔴 {crit+low} {t('products need reordering','منتجات تحتاج إعادة طلب')}</div>",
                        unsafe_allow_html=True)
