# ╔══════════════════════════════════════════════════════════════════╗
# ║         🏢 SWAG Dashboard – Secrets Edition                      ║
...
# ─────────────────────────────────────────────────────────────────────────────
# MINIMAL CUSTOM CSS  (fonts only – Streamlit default white theme for everything else)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# SECRETS LOADER  (called once at entry point – never at module level)
# ─────────────────────────────────────────────────────────────────────────────

# Populated by _load_secrets() at the entry point before any page renders.
ODOO_SYSTEMS: dict = {}
_LOGIN_URL:   str  = ""
_LOGIN_DB:    str  = ""
...
def _load_secrets() -> tuple:
    """
    Read all credentials from st.secrets at runtime.
    Shows a friendly setup guide and calls st.stop() if any key is missing.

    Required secrets.toml layout:
    ─────────────────────────────
    [LOGIN]
...
""",
    ...
    return systems, st.secrets["LOGIN"]["url"], st.secrets["LOGIN"]["db"]
...
def odoo_login(email: str, password: str) -> int:
    """Authenticate against the LOGIN Odoo instance (email + password)."""
    payload = {
        "jsonrpc": "2.0",
        "method": "call",
        "params": {
...
def page_multi_odoo_compare():
    ...
    with col_right:
        with st.container(border=True):
            ...
            if "last_compare_meta" in st.session_state:
                ...
                m1, m2 = st.columns(2)
                m1.metric(t("Models checked", "الموديلات"), meta.get("models", 0))
                m2.metric(t("Systems online", "الأنظمة"),   f"{meta.get('systems_ok', 0)}/3")
                st.caption(
                    f"SWAG: **{meta.get('swag_status','?')}** · "
                    f"La Rouche: **{meta.get('lr_status','?')}** · "
                    f"Diff. Clothes: **{meta.get('dc_status','?')}**"
                )
            else:
                st.info(t("No comparison run yet.", "لم يتم تشغيل أي مقارنة بعد."))
    ...
    st.session_state["last_compare_meta"] = {
        "models":      len(models),
        "systems_ok":  int(df_all.loc[df_all["Product"] != "(not connected)",
                                      "System"].nunique()),
...
def login_page():
    ...
    with col:
        st.markdown(
            f"<h1 style='text-align:center'>🏢 {t('SWAG Dashboard','لوحة تحكم سواغ')}</h1>"
            f"<p style='text-align:center;color:gray;font-size:.9rem'>"
...
# ─────────────────────────────────────────────────────────────────────────────
# ENTRY POINT  – secrets loaded here, before any page renders
# ─────────────────────────────────────────────────────────────────────────────
